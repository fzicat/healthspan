...
export default function TrainingPage() {
    ...
    return <main className="max-w-3xl mx-auto py-6 pb-20 space-y-5">
        <header className="flex items-start justify-between gap-3"><div><Link href="/training" className="inline-flex min-h-11 items-center text-primary underline">Back to training</Link><h1 className="text-2xl font-bold">Training confirmations</h1><p className="text-sm text-muted-foreground">Transitional authenticated tools. Discuss decisions with Dozer first; a conversation alone does not activate a plan.</p></div><button className={trainingButton} onClick={() => void refresh()} disabled={loading}>Refresh</button></header>
