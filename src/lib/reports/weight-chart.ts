export const WEIGHT_TIMEFRAMES = ['1W', '1M', '3M', '6M', '1Y', 'All'] as const
export type WeightTimeframe = typeof WEIGHT_TIMEFRAMES[number]
...
export type WeightAveragePoint = WeightPoint & {
    average: number
    sampleCount: number
}
...
export function getWeightRangeStart(anchorDate: string, timeframe: WeightTimeframe): string | null {
    switch (timeframe) {
        case '1W':
...
export function getWeightQueryStart(displayRangeStart: string | null): string | null {
    return displayRangeStart === null ? null : subtractDays(displayRangeStart, 6)
}

export function calculateSevenDayMovingAverage(points: WeightPoint[]): WeightAveragePoint[] {
    const sorted = [...points].sort((a, b) => a.date.localeCompare(b.date))
    const result: WeightAveragePoint[] = []
    ...
    for (let index = 0; index < sorted.length; index += 1) {
        ...
        const earliestIncludedTime = pointTime - (6 * DAY_MS)
        windowSum += point.weight

        while (
            windowStartIndex < index
            && parseDate(sorted[windowStartIndex].date).getTime() < earliestIncludedTime
        ) {
            windowSum -= sorted[windowStartIndex].weight
            windowStartIndex += 1
        }
